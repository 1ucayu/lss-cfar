...
from cnn_model import CFARCNN  # Importing the CNN model
from cnn_dataset import CNNDataset  # Importing the dataset
from cnn_args import get_args  # Importing the argument parser
...
def generate_checkpoint_name(args):
    ...
    return (
        f"cnn_"
        f"{timestamp}_"
        f"conv_layers_{5}_filters_{64}_lr_{args.learning_rate}_batch_{args.batch_size}_steps_{args.total_steps}_"
...
def train():
    ...
    # Create model
    model = CFARCNN().to(device)
    
    # Load dataset
    train_dataset = CNNDataset(phase='train', dataset_paths=args.dataset_paths, calibration_paths=args.calibration_paths)
    val_dataset = CNNDataset(phase='test', dataset_paths=args.dataset_paths, calibration_paths=args.calibration_paths)
